...
| `docx/` | The companion documentation set — 25 numbered documents (Project Proposal → UI-UX Design; 19 = Clearing & Forwarding spec, 20 = Warehouse Management spec, 21 = Multi-Site Collateral Management spec, 22 = Delivery Management System spec, 23 = Milling ERP & Integrated Business Management System spec, 24 = Social Media Publishing spec, 25 = Research & Consultancy Management Information System (RC-MIS) spec), plus `morise.docx`, `Multi_Holdings_Limited_Software_Development_Procedures.docx`, **`How to Run the System (Local Development) - Morise Holdings Limited.docx`** (practical run steps for this proof-of-concept, restating `mbms/README.md`'s "Running it" section in house documentation format — not part of the dated status-log routine), the raw client attachments (`cf.docx`, `WMS_Features_Uganda_Multi_Branch.docx`, `Multi-Site-Collateral-Management-System-Feature-Specification.docx`, `Delivery-Software-Feature-Specification.docx`, `Milling_Company_Software_Development_Features.docx`, `Research_and_Consultancy_Software_Development_Features.docx`), and dated backups in `docx_backup*/`. |
...
## Staff Identification Card — Back Face Sized to the Front, Blood Group Removed (3 September 2026)

Two adjustments to the card back from Update 91, at an administrator's
request:

- **Blood group removed.** The card back no longer shows a blood group. The
  card-held **`blood_group`** column is **dropped** (migration
  `20260903160000_staff_id_card_drop_blood_group`); the free-text
  **`back_notes`** field is kept. The **`back`** object (`GET
  /api/v1/staff-id-cards*`, `GET /api/v1/my-hr/id-card`) no longer carries
  `bloodGroup`, and `POST` / `PATCH /api/v1/staff-id-cards/:id` no longer
  accept it. The back still shows the holder's **national ID** and
  **emergency contact** (from the `Employee`), the **conditions of use**,
  the **“if found, return to”** block, an optional **note** and the
  **barcode**. The admin **Edit** dialog drops the *Blood group* field.
- **Back sized to the front.** `IdCard.jsx` now renders the **front and the
  back at the same width and height** — a fixed `CARD_W × CARD_H` box with
  overflow clipped, the back's barcode and signature lines anchored to the
  bottom edge.
- **No new endpoint or permission**; `openapi.json` path / operation counts
  unchanged (**367 / 467**). Verified end to end against the running system
  (the migration drops the column; the card and `back` payloads no longer
  include `bloodGroup`; `backNotes` still round-trips; both faces render at
  the same size). `nest build` + admin `vite build` + `prisma migrate
  deploy` clean. Recorded as `Implementation Status Update 92` (docs 02–18);
  **Addendum (3 September 2026)** on docs 19–21; `Implementation Status
  Note 69` in
  `Multi_Holdings_Limited_Software_Development_Procedures.docx`;
  `morise.docx` as **Update 49**; `01_Project Proposal` (LibreOffice-locked)
  staged as `*.PENDING_Update92.txt`. Pre-edit copies in `docx_backup81/`.

---

## Morise Milling Ltd — sixth subsidiary + Milling ERP specification (new doc 23) (3 September 2026)

An administrator asked to add a subsidiary under Morise Holdings Limited
with a **grain-milling** concept, based on the client attachment
`docx/Milling_Company_Software_Development_Features.docx`.

- **Subsidiary seeded** (`prisma/seed.ts` only) — **Morise Milling Ltd**, a
  wholly-owned (100%) subsidiary (company
  `a1000000-0000-4000-8000-000000000007`), currency UGX, FY starting
  1 January 2026, at Plot 15, Nalukolongo Industrial Area, Kampala:
  - **6 mill sites / branches** (`b1…0040`–`0045`): Kampala Head Office &
    Central Mill, Jinja Wheat & Maize Mill, Masindi Maize Mill, Mbale
    Millet & Sorghum Mill, Lira Grain Mill & Silos, Kampala Distribution
    Depot;
  - **12 departments** (`d1…0060`–`0071` — `…0040`–`0042` are Morise Agro
    Ltd's), one per architecture module: Grain Procurement & Weighbridge,
    Warehouse & Inventory, Production & Milling Lines, Machine &
    Maintenance, Quality Assurance & Control, Packaging, Sales & Customer
    Management, Distribution & Delivery, Finance & Accounting, Human
    Resources & Workforce, Procurement & Supplier Management, IT / IoT &
    Integrations;
  - **6 starter policies** (`p1…0040`–`0045`): Weighbridge Ticket
    Integrity, Grain Acceptance & Moisture Limits, Milling Yield & Loss
    Tolerance, Batch Traceability End to End, Preventive Maintenance
    Compliance, Food Safety & Product Release.
  - **No code, schema or permission change** — it is a real, manageable
    subsidiary from day one under the subsidiary-lifecycle capability.
- **New `docx/23` — Milling ERP & Integrated Business Management System —
  Feature Specification** restates the attachment in the house format:
  **twelve feature modules** (grain procurement over a weighbridge;
  warehouse & inventory; production & milling lines; machine &
  maintenance; QA/QC; packaging; sales & customers; distribution &
  delivery; finance & accounting; HR & workforce; procurement & suppliers;
  and a management dashboard with **IoT / PLC integration** and
  **end-to-end supplier → grain-lot → milling-batch → packaging →
  customer traceability**), plus non-functional requirements, an
  architecture overview, an integration table, the fifteen recommended
  roles, a technology-stack table, the organisation as seeded, a
  **five-phase delivery roadmap** and a section on **reuse of the existing
  MBMS modules** (Suppliers, Inventory / Warehouse, Fleet, Delivery,
  Accounting, HR / Payroll, Alerts, Audit). **The milling system is not
  built.**
- **Storefront** — the public site lists **Morise Milling Ltd** in the
  Companies directory: a `SUBSIDIARY_META` entry, a new
  `brand/banners/subsidiary-milling.svg` in the brand family (mirrored into
  `mbms/frontend/public/` and `mbms/storefront/public/`), and a News item.
- **Group figures become 6 subsidiaries / 26 sites / branches**
  (catalogue unchanged — Morise Milling Ltd is not a storefront seller
  yet). Verified against the running system: the `/organization` API
  returns the company, its 6 branches, 12 departments and 6 policies and
  they are manageable by the Managing Director; admin + storefront `vite
  build` clean. Recorded as `Implementation Status Update 93` (docs 02–18)
  with per-document *Impact* lines; **Addendum (3 September 2026)** on docs
  19–22; `Implementation Status Note 70` in
  `Multi_Holdings_Limited_Software_Development_Procedures.docx`;
  `morise.docx` as **Update 50**; `01_Project Proposal`
  (LibreOffice-locked) staged as `*.PENDING_Update93.txt`. Pre-edit copies
  in `docx_backup82/`.

---

## Automatic Morise e-Stamp on a customer-acknowledged delivery (3 September 2026)

**Implements** the concept recorded as spec-only in Update 66 / the doc 22
addendum. Once a Morise delivery is **completed**, the customer confirms —
through the customer portal — that they received the order's goods /
services **in good condition**, and MBMS **automatically places a Morise
e-Stamp on the order's invoice**.

- **New endpoint** — `POST /api/v1/customer-portal/orders/:id/delivery/acknowledge`
  (customer JWT), body `{ condition: good | damaged | incomplete |
  not_received, note? }`. Allowed only once the order's delivery is
  `delivered`, and only once.
- **`good` → e-Stamp.** In one transaction the order's invoice is stamped:
  **`invoices.stamped_at`**, a unique **`invoices.stamp_number`** =
  `MOR-ESTAMP-YYYY-NNNNNN`, **`invoices.stamp_condition`** = `good` (new
  nullable columns, migration `20260903180000_invoice_estamp_delivery_ack`);
  a delivery event, an `invoice.estamp.issued` audit entry and a best-effort
  notification to the selling subsidiary's order team are recorded.
- **Anything else → delivery exception, no stamp.** `damaged` /
  `incomplete` / `not_received` sets **`deliveries.exception_opened_at`**,
  logs the customer's report as a delivery event and raises a
  `delivery.exception_opened` audit entry + staff notification. The
  delivery records `customer_ack_condition` / `customer_ack_at` /
  `customer_ack_note` / `customer_ack_by` in every case.
- **Shown on** — the customer delivery-tracking view (`GET
  .../orders/:id/delivery` now returns `canAcknowledge`, the
  acknowledgement fields and, once stamped, `eStamp`), the invoice list /
  detail (`eStamped`, `stampNumber`), the staff `DeliveryService.get`, and
  the **downloaded invoice PDF** — which now draws a **navy / amber Morise
  seal** (the Morise "M" mark, `MORISE e-STAMP`, the stamp number and the
  confirmation wording) top-right.
- **Storefront** — the *My Orders* detail shows a **"Did everything arrive
  in good condition?"** panel for a completed delivery (**Yes — confirm
  receipt** / **Report a problem** with a reason + note); afterwards it
  shows the confirmation and the e-Stamp number, or the exception notice.
  The *Invoices* list gains a **"Morise e-Stamp"** badge.
- **Seed** — one completed delivery (`DEL-2026-0001`) for a Highland Traders
  order, left un-acknowledged, so the flow is demoable.
- **No new permission.** Verified end to end against the running system (a
  `good` acknowledgement issued `MOR-ESTAMP-2026-000001` and rendered the
  seal on the PDF; `not delivered` → 409, `already acknowledged` → 409, `no
  delivery` → 404; `damaged` → exception opened, no stamp). `nest build` +
  storefront `vite build` + `prisma migrate deploy` clean; `openapi.json`
  regenerated (**368 paths / 468 operations**). Recorded as `Implementation
  Status Update 94` (docs 02–18); a **built** addendum on `docx/22` +
  cross-ref addenda on docs 19–21 and 23; `Implementation Status Note 71` in
  `Multi_Holdings_Limited_Software_Development_Procedures.docx`;
  `morise.docx` as **Update 51**; `01_Project Proposal` (LibreOffice-locked)
  staged as `*.PENDING_Update94.txt`. Pre-edit copies in `docx_backup83/`.

---

## A clear "Confirm receipt" button / link for the customer e-Stamp confirmation (3 September 2026)

Follow-up to Update 94: the customer confirmation that triggers the Morise
e-Stamp is now a **first-class call to action**, not a panel buried in an
order's detail.

- **New read** — `GET /api/v1/customer-portal/orders/awaiting-confirmation`
  (customer JWT) returns the customer's own orders with a **completed
  delivery they have not yet confirmed** (`orderId`, `orderNumber`,
  `deliveryNumber`, `deliveredAt`, `companyName`). The dashboard summary
  (`GET /customer-portal/dashboard-summary`) gains **`deliveriesToConfirm`**
  and **`firstDeliveryToConfirmOrderId`**. **No schema change, no new
  permission.**
- **Dedicated page** — **`/orders/:id/confirm`**, a shareable URL: the order
  + delivery summary, a prominent **"Confirm — received in good condition"**
  button and a **"Report a problem"** form (damaged / incomplete / not
  received + note). On a good confirmation it shows the **Morise e-Stamp
  number** and a **"Download stamped invoice"** link; on a problem, the
  exception notice.
- **Surfaced where the customer sees it** — the *My Orders* list marks each
  pending order with a **"Confirm receipt →"** link, and the **Home
  dashboard** shows a prompt card (*"You have N deliveries awaiting your
  confirmation"* + **"Confirm now"**).
- The Update 94 acknowledge endpoint, the automatic e-Stamp and the
  in-detail panel are **unchanged**. Verified end to end
  (`awaiting-confirmation` + the dashboard count move 1 → 2 → 1 as a
  delivery is completed and confirmed; the page issues the e-Stamp). `nest
  build` + storefront `vite build` clean; `openapi.json` regenerated (**369
  paths / 469 operations**). Recorded as `Implementation Status Update 95`
  (docs 02–18); an addendum on `docx/22` + cross-ref addenda on docs 19–21
  and 23; `Implementation Status Note 72` in
  `Multi_Holdings_Limited_Software_Development_Procedures.docx`;
  `morise.docx` as **Update 52**; `01_Project Proposal` (LibreOffice-locked)
  staged as `*.PENDING_Update95.txt`. Pre-edit copies in `docx_backup84/`.

---

## Confirm-receipt button driven by the order's delivered status (3 September 2026)

Refinement of Updates 94–95: the customer's **"Confirm receipt"** button is
now driven by the **order's status set by admin staff** — once staff mark
an order **delivered**, the button appears in *My Orders* and the customer
clicks it to attest good condition, which generates the automatic Morise
e‑Stamp. **A Morise Logistics delivery record is no longer required.**

- **`Order` gains** — **`delivered_at`** (stamped when staff advance the
  order to `delivered`, or a Morise Logistics delivery completes) and
  **`customer_receipt_confirmed_at`** / **`customer_receipt_condition`** /
  **`customer_receipt_note`** (migration
  `20260903200000_order_customer_receipt`, which back‑fills `delivered_at`
  for orders already delivered).
- **Reads** — `GET /api/v1/customer-portal/orders` + `.../:id` now return
  `deliveredAt`, the receipt fields, **`canConfirmReceipt`**
  (`order.status === 'delivered'` and not yet confirmed) and
  `eStamped` / `stampNumber` on the invoice sub‑object.
- **`POST /api/v1/customer-portal/orders/:id/delivery/acknowledge`** is now
  **order‑gated**: allowed once the order is `delivered` and only once,
  with or without a delivery record. **`good`** records the receipt on the
  order and auto‑stamps its invoice (`MOR-ESTAMP-YYYY-NNNNNN`); any other
  condition records a **dispute** (no stamp). When a Morise Logistics
  delivery record exists, the confirmation is **mirrored onto it**
  (`customer_ack_*` + a delivery event). Audited as `invoice.estamp.issued`
  / `order.receipt.disputed` with a best‑effort staff notification.
  `awaiting-confirmation` + the dashboard **`deliveriesToConfirm`** count
  are now **order‑driven**.
- **Storefront** — the *My Orders* panel, the `/orders/:id/confirm` page and
  the **"Confirm receipt →"** row link all key off the order's delivered
  status. **No new endpoint or permission.**
- **Seed** — `delivered_at` is set on every delivered order and the settled
  ones are pre‑confirmed + e‑stamped, leaving one order (`ORD-2026-0001`,
  which carries `DEL-2026-0001`) still awaiting the customer's confirmation.
  Verified end to end (staff advanced an order to delivered; the customer
  confirmed `damaged` → dispute, no stamp, and `good` → e‑Stamp, mirrored
  onto the delivery record where present; `not delivered` → 409, `already
  confirmed` → 409). `nest build` + storefront `vite build` + `prisma
  migrate deploy` clean; `openapi.json` unchanged (**369 paths / 469
  operations**). Recorded as `Implementation Status Update 96` (docs
  02–18); an addendum on `docx/22` + cross‑ref addenda on docs 19–21 and
  23; `Implementation Status Note 73` in
  `Multi_Holdings_Limited_Software_Development_Procedures.docx`;
  `morise.docx` as **Update 53**; `01_Project Proposal` (LibreOffice-locked)
  staged as `*.PENDING_Update96.txt`. Pre-edit copies in `docx_backup85/`.

---

## Social media publishing — composer, publishing engine & status dashboard (new doc 24) (3 September 2026)

A mini **Buffer / Hootsuite** inside MBMS: write **one master post**, fan it
out to many social media channels in **one click** with per‑platform caption
overrides, publish now or schedule, and watch per‑platform success / failure.
The architecture plan an administrator supplied is restated in house format in
the **new `docx/24` — *Social Media Publishing — Composer, Publishing Engine &
Status Dashboard — Feature Specification***.

- **New module `src/social-publishing`** exposes **`/api/v1/social`**.
- **Schema** (migration `20260903220000_social_publishing`) —
  **`social_connections`** (one simulated OAuth connection per platform:
  status, account label, a ~60‑day token expiry, a placeholder token
  reference — no real handshake), **`social_posts`** (master body, media
  type + text‑reference URL, link, status, `scheduledFor`) and
  **`social_post_targets`** (one row per platform per post — caption
  override, status, external id + permalink, error, attempts).
- **Per‑platform rules** — `PLATFORM_RULES` carries the character limit,
  media requirement and truncation behaviour for facebook / x / instagram /
  linkedin / youtube / tiktok / whatsapp / telegram / other; `GET
  /social/posts/:id` returns a live per‑target preview (effective caption,
  char count vs limit, over‑limit / will‑truncate flags, media‑rule and
  connection warnings).
- **Endpoints** — `GET /social/platforms`;
  `POST /social/platforms/:platform/{connect,disconnect}`;
  `GET`/`POST /social/posts`, `GET`/`PATCH`/`DELETE /social/posts/:id`;
  **`POST /social/posts/:id/publish`** (the single button),
  `POST /social/posts/:id/retry` (failed targets only),
  `POST /social/run-scheduled` (cron stand‑in) and `GET /social/dashboard`.
  Every mutation is audited.
- **Permissions** (governance domain) — **`social.post.manage`** (draft /
  edit / delete, connect / disconnect channels), **`social.post.publish`**
  (publish, retry, run scheduled) and **`social.post.viewAll`** (composer,
  posts, dashboard) — the *"can create drafts vs can publish"* split.
  manage+publish+viewAll → Super Administrator / Managing Director / IT
  Administrator; Sales Manager gets manage+viewAll; oversight roles (Group
  CEO, Auditor, External Auditor, Board Member, Legal Officer, Read‑only
  User) get viewAll.
- **Admin app** — a new **Social Publishing** screen under **ADMIN /
  BACKEND** (four tabs: Compose, Posts, Channels, Dashboard) with a live
  per‑channel preview and caption overrides.
- **Simulated** — no OAuth handshake, platform API call or job queue exists
  in this build (the same documented deferral as Kafka, object storage and
  email / SMS); the connection / post / target / partial‑failure / retry /
  dashboard shape is real. `dispatchOne` fakes an `externalId` + permalink
  on success and fails a target that is disconnected, over a hard limit, or
  missing required media.
- Verified end to end against the running system (9 platforms, 4 connected;
  a fan‑out with Instagram missing media → `failed` → post
  `partially_failed`; adding media + retry → `published`; disconnect → next
  publish fails that target; `run-scheduled` fired a due post; Sales Manager
  `POST /social/posts` → 403, Auditor `GET` 200 / `POST` 403). `nest build`
  + admin `vite build` + `prisma migrate deploy` clean; `openapi.json`
  regenerated (**378 paths / 481 operations / 188 schemas**). Recorded as
  `Implementation Status Update 97` (docs 02–18); cross‑ref addenda on docs
  19–23; `Implementation Status Note 74` in
  `Multi_Holdings_Limited_Software_Development_Procedures.docx`;
  `morise.docx` as **Update 54**; `01_Project Proposal` (LibreOffice-locked)
  staged as `*.PENDING_Update97.txt`. Pre-edit copies in `docx_backup86/`.

---

## Product catalogue for Morise Milling Ltd + branch-tagged product registration (doc 23) (8 September 2026)

An administrator asked to add products to **Morise Milling Ltd** — the sixth
subsidiary (Update 93, 3 September 2026) was seeded with six mill sites and
twelve departments but **zero products**. This pass seeds its first product
catalogue and generalises product registration so any subsidiary's Admin
Products screen can tag a product to the branch/site that makes it.

- **`prisma/seed.ts` gains 24 SKUs across 7 new categories** for Morise
  Milling Ltd: **Maize Milling Products** (5 — Grade 1 sifted flour in
  50/25/10/2 kg, Grade 2 whole maize meal), **Wheat Milling Products** (3 —
  bakers'-grade, home-baking, whole-wheat atta), **Millet & Sorghum
  Products** (3 — millet flour, sorghum flour, a fortified composite
  porridge flour), **By-Products & Animal Feed** (4 — maize bran, wheat
  bran/pollard, maize germ, a compounded dairy meal), **Raw Grain (Intake)**
  (3), **Packaging Materials** (2) and **Milling Services** (4 — toll
  milling, grain cleaning/drying, silo storage, grading & moisture testing).
  Each item is attributed to the mill or depot that produces or holds it
  (Masindi maize, Jinja wheat, Mbale millet/sorghum, Lira grain & silos, the
  Kampala depot and head office).
- **Raw grain and packaging (5 items) carry no `unitPrice` on purpose** —
  `CustomerCatalogService` only lists priced products, so these internal
  stock items stay off the customer storefront while Admin Inventory still
  tracks their stock, reorder point and min/max levels. `trackBatches` is
  set on every grain-derived item per docx/23 §2.2 and §2.5–2.6 (raw /
  finished-goods / packaging warehouses, FIFO-FEFO, batch numbers, expiry /
  best-before, end-to-end traceability).
- **Product registration generalised** — `Product.branchId` has existed on
  the schema since 29 August 2026, but only the seed could set it.
  `CreateProductDto`/`UpdateProductDto` gain an optional `branchId`,
  validated by a new `ProductsService.assertReferencesBelongTo` to belong to
  the product's own `companyId` (same check now covers `categoryId`); every
  product read returns `companyName`/`branchName`.
- **Admin Products screen rebuilt** — a subsidiary filter above the list;
  Company / Branch / Stock columns and a per-row **Edit** action; the New
  Product form's bare company-UUID text field replaced with linked
  Company → Branch → Category dropdowns; a new **Edit Product** modal wired
  to `PATCH /products/:id` (live since Sprint 4, no screen before now).
- No new endpoint, migration or permission. Verified end to end — reseeded,
  all 24 Morise Milling Ltd rows confirmed in Postgres with correct branch
  attribution; `nest build` clean; `openapi.json` regenerated **unchanged**
  at **378 paths / 481 operations / 188 schemas** (fields added to an
  existing schema, no new path). Group catalogue → **58 products / 25
  categories** (53 storefront-priced, 5 internal-only); **all six operating
  subsidiaries now sell at least one product or service**. Recorded as
  `Implementation Status Update 98` (docs 02–18); **doc 23** gains a full
  **"Product Catalogue Seeded for Morise Milling Ltd"** addendum, cross-ref
  addenda on docs 19–22; `Implementation Status Note 75` in
  `Multi_Holdings_Limited_Software_Development_Procedures.docx`;
  `morise.docx` as **Update 55**; `01_Project Proposal` (LibreOffice-locked)
  staged as `*.PENDING_Update98.txt`. Pre-edit copies in `docx_backup87/`.

---

## Morise Research & Consultancy Ltd — seventh subsidiary + RC-MIS specification (new doc 25) (9 September 2026)

An administrator asked to add a subsidiary for research and other types of
consultancy, based on the client attachment
`docx/Research_and_Consultancy_Software_Development_Features.docx`.
**Morise Research & Consultancy Ltd** is seeded as the group's **seventh
wholly-owned (100%) subsidiary**, the same low-footprint way as every
subsidiary before its first feature build — organisation only, system left
as a specification.

- **Company** — id `a1000000-0000-4000-8000-000000000008`, registered at
  Plot 7, Acacia Avenue, Kololo, Kampala, currency UGX, financial year
  starting 1 January 2026, created and managed by the Managing Director
  under the existing subsidiary-lifecycle capability (Update 34).
- **Five sites / branches** (`b1…0050`–`0054`) — Kampala Head Office, and
  four regional field offices reaching the study areas the client
  attachment's geographic-coverage requirements call for: Gulu (Northern),
  Mbale (Eastern), Mbarara (Western), and a Karamoja Field Station (Moroto).
- **Twelve departments** (`d1…0080`–`0091`) condensing the client's
  fourteen-node RC-MIS architecture into standing organisational units:
  Client & Business Development; Contracts & Grants Management; Research
  Design & Methodology; Field Operations & Data Collection; Data Quality &
  Management; Data Analysis & Qualitative Research; Monitoring &
  Evaluation; Reports, Deliverables & Knowledge Management; Ethics &
  Compliance; Finance, Procurement & Subcontracting; Human Resources &
  Consultant Management; IT, Client Portal & Systems.
- **Six starter policies** (`p1…0050`–`0055`) — Research Ethics & Informed
  Consent, Data Protection & Participant Confidentiality, Fieldwork Safety &
  Security, Data Quality Assurance, Consultant & Enumerator
  Conflict-of-Interest, and Contract, Grant & Proposal Compliance.
- **seed.ts only** — no code, schema or permission change. **Not a
  storefront seller** (0 products) — a research and consultancy firm is
  billed on contracts, retainers and timesheets, not a priced catalogue, so
  it does not get a products pass the way Morise Milling Ltd did.
- **The RC-MIS itself is a specification, not built** — **new `docx/25` —
  *Research & Consultancy Management Information System (RC-MIS) — Feature
  Specification*** restates the client's fourteen feature modules (CRM;
  Proposals & Tenders; Contracts & Grants; Research Projects; Field Data
  Collection & Sampling; Data Quality, Management & Analysis; Qualitative
  Research; Monitoring & Evaluation; Reports, Deliverables & Knowledge
  Management; Ethics, Risk & Compliance; Consultants, Researchers & HR;
  Project Finance, Timesheets & Invoicing; Client Portal & Communication;
  Management Analytics & Security) in house format, plus non-functional
  requirements, a system-architecture overview, key integration points
  (mobile/offline data collection, GPS, SPSS/Stata/R/Python, Power BI /
  Tableau, e-signature, payment gateway, file storage), fourteen user
  roles, a suggested technology stack, the organisation as built above, a
  five-phase delivery roadmap, and how it would reuse the existing MBMS
  platform.
- **Storefront corporate site** picks the new subsidiary up automatically
  from `GET /customer-portal/public/overview`; `corporate.js` gains a
  `SUBSIDIARY_META` entry (*"Research & consultancy"*, Kololo, Kampala), a
  `SUBSIDIARY_BANNER` entry and a News item. A new
  `brand/banners/subsidiary-research-consultancy.svg` — **indigo accent**
  (the group's first new subsidiary hue since Milling reused the base
  amber), a bar-chart-and-magnifying-glass motif — is mirrored into both
  apps' `public/brand/banners/` and added to `brand/README.md`'s banner
  table.
- Verified end to end against the running system — the company, its 5
  branches, 12 departments and 6 policies confirmed in Postgres and via the
  authenticated Admin organisation API; the public overview endpoint lists
  it with its branch count; both banner assets resolve on the Admin app and
  the Customer Storefront. **No new endpoint, migration or permission**;
  `openapi.json` unchanged. Group now **7 subsidiaries, 31 sites /
  branches**. Recorded as `Implementation Status Update 99` (docs 02–18);
  cross-ref addenda on docs 19–24; `Implementation Status Note 76` in
  `Multi_Holdings_Limited_Software_Development_Procedures.docx`;
  `morise.docx` as **Update 56**; `01_Project Proposal` (LibreOffice-locked)
  staged as `*.PENDING_Update99.txt`. Pre-edit copies in `docx_backup88/`.

---

## Services catalogue for Morise Research & Consultancy Ltd (doc 25) (9 September 2026)

An administrator asked to add products to Morise Research & Consultancy
Ltd. The subsidiary was seeded a day earlier (Update 99) with **zero
products**, on the reasoning that a consultancy is billed on contracts and
timesheets. Following the precedent of **Morise Collateral Management
Ltd** — also contract-billed, but carrying a priced services catalogue
since Update 60 — this pass seeds its first catalogue.

- **`prisma/seed.ts` gains 23 services (`RC-7001`–`RC-7051`) across 6 new
  categories**, reusing the same `ensureService()` helper already
  exercised for the Collateral Management catalogue: **Research &
  Evaluation Studies** (6 — baseline, midline/endline, impact assessment,
  feasibility, market research, needs assessment), **Field Data
  Collection** (4 — household-survey enumeration, FGDs/KIIs, mobile/
  offline data-collection setup, enumerator recruitment & training),
  **Data Management & Analysis** (4 — cleaning, statistical analysis,
  qualitative coding, dashboard setup), **Monitoring & Evaluation
  Services** (3 — M&E framework design, indicator-tracking-system setup,
  ongoing M&E support), **Technical & Policy Consultancy** (4 — policy
  advisory, institutional capacity assessment, strategic planning
  facilitation, a senior-researcher day rate) and **Training & Capacity
  Building** (2 — research-methods and M&E workshops).
- **Every line is `service` and carries a `unitPrice`** — quoted in UGX
  against the standard unit a client's statement of work would use (per
  study, per session, per dataset, per day) — so, unlike Morise Milling
  Ltd's internal raw-material items, the **entire catalogue is
  storefront-visible**.
- **Branch attribution reflects where the work happens**: study design,
  analysis, policy advisory and the day-rate line sit at the Kampala Head
  Office; fieldwork-heavy lines (household-survey enumeration, focus
  groups, enumerator training, a needs assessment, ongoing M&E support,
  and the two training workshops) are spread across the Gulu, Mbale,
  Mbarara and Karamoja field offices.
- **Seed-only** — no schema, code, endpoint or permission change; reuses
  the existing `ensureService()`/`ensureCategory()` helpers exactly as
  every prior services catalogue (Logistics, Clearing & Forwarding,
  Warehouse Management, Collateral Management).
- Verified end to end against the running system — all 23 rows confirmed
  in Postgres with correct branch/category attribution; the Admin
  Products screen, filtered to Morise Research & Consultancy Ltd, lists
  all 23 services correctly (screenshot-verified, no console errors);
  `nest build` clean; `openapi.json` regenerated **byte-identical**
  (**378 paths / 481 operations / 188 schemas**). Group catalogue → **81
  products / 31 categories** (76 storefront-priced, 5 internal-only);
  **all seven operating subsidiaries now sell at least one product or
  service**. Recorded as `Implementation Status Update 100` (docs
  02–18); **doc 25** gains a full **"Services Catalogue Seeded for
  Morise Research & Consultancy Ltd"** addendum, cross-ref addenda on
  docs 19–24; `Implementation Status Note 77` in
  `Multi_Holdings_Limited_Software_Development_Procedures.docx`;
  `morise.docx` as **Update 57**; `01_Project Proposal`
  (LibreOffice-locked) staged as `*.PENDING_Update100.txt`. Pre-edit
  copies in `docx_backup89/`.

---

## Dockerized backend, Admin app and Customer Storefront (9 September 2026)

An administrator asked to dockerize the backend, the Admin app and the
Customer Storefront. Until now only Postgres and Redis ran as containers
(`mbms/docker-compose.yml`); the three application services ran natively.

- **`backend/Dockerfile`** — a two-stage build on **`node:20-bookworm-slim`**
  for *both* stages, deliberately not Alpine: Prisma's query-engine binary
  is glibc-linked, and mixing an Alpine (musl-libc) build stage with a
  glibc runtime — or vice versa — is a well-known way to get a "Query
  Engine could not be found" failure at container start that never shows
  up locally. `npm ci` → `prisma generate` → `npm run build` (**`nest
  build`, not a bare `tsc`**, so the `@nestjs/swagger` CLI plugin still
  populates the OpenAPI document's request/response schemas exactly as it
  does outside Docker); the runtime stage copies over only the compiled
  `dist/`, the already-generated Prisma Client, and production
  `node_modules` (`npm ci --omit=dev`). Both stages install `openssl` and
  `ca-certificates` explicitly — Prisma's own documented fix for a
  libssl-detection warning on the bare `-slim` image.
- **`frontend/Dockerfile`** and **`storefront/Dockerfile`** — a matching
  two-stage build on `node:20-alpine` → `nginx:1.27-alpine`: `npm run
  build` (Vite) produces a static bundle, and a bundled `nginx.conf`
  reverse-proxies `/api/*` to the `backend` container by its Compose
  service name — the **same relationship `vite.config.js`'s dev-server
  proxy** gives each app locally, so no code change and no CORS
  configuration are needed either way. A `try_files … /index.html`
  fallback handles client-side routing.
- **`docker-compose.yml`** gains `backend`/`frontend`/`storefront`
  services: `backend` waits on a new Postgres **healthcheck**
  (`pg_isready`) before starting, and its `DATABASE_URL`/`PORT` are
  overridden to the in-network Postgres host and internal port (everything
  else still comes from `backend/.env` via `env_file`); `frontend` and
  `storefront` are exposed on the **same host ports their Vite dev servers
  already used** (5173 / 5174) so the URLs are unchanged either way.
- **Migrations and seeding stay a host-run step** —
  `npx prisma migrate deploy && npx prisma db seed` from `backend/` against
  the same containerized Postgres — unchanged from before the backend
  itself was containerized, since the runtime image omits the `prisma` CLI
  and `ts-node` to stay small.
- **No schema, endpoint or permission change**; `openapi.json` unchanged
  (**378 paths / 481 operations / 188 schemas**).
- Verified end to end — all three images build clean;
  `docker compose up -d --build` brings up all five containers, `backend`
  reporting healthy against Postgres's Compose healthcheck; existing
  seeded data (**8 companies, 81 products**) survived a Postgres container
  recreate because it lives in the `mbms_pg_data` **named volume**, not the
  container itself; a staff login through the dockerized Admin app and a
  page load of the dockerized Customer Storefront both succeeded with the
  request round-tripping through nginx to the `backend` container and
  back, with **zero browser console errors** on either app
  (screenshot-verified). Recorded as `Implementation Status Update 101`
  (docs 02–18); cross-ref addenda on docs 19–25; `Implementation Status
  Note 78` in `Multi_Holdings_Limited_Software_Development_Procedures.docx`;
  `morise.docx` as **Update 58**; `01_Project Proposal`
  (LibreOffice-locked) staged as `*.PENDING_Update101.txt`; the standalone
  `docx/How to Run the System (Local Development).docx` gains a new
  "Running it fully dockerized" section. Pre-edit copies in
  `docx_backup90/`.

---

## Storefront language set scoped to 9 official languages (9 September 2026)

An administrator asked for the Customer Storefront to be localised and
globalised into a specific, named set of languages, grouped exactly as
given: Uganda — Official/Global (English), Uganda — Official/Africa
(Kiswahili), Uganda — Bantu (Luganda), Africa (Arabic), Europe (German,
French, Spanish, Italian), and The Americas (Portuguese).

- The storefront's language picker already shipped these same 9 languages
  fully translated (added across Updates 29–31), inside a much larger
  **43-language** aspirational list — 34 English-fallback placeholder
  languages plus two partially-translated regional languages (Runyankole,
  Acholi), added Update 30. `mbms/storefront/src/locales/index.js`
  `LANGUAGE_LIST` and `LOCALES` are now scoped to **exactly these 9** — the
  administrator's official set — retiring the placeholder and partial
  entries; every remaining language ships a complete translation, so the
  picker's "English for now" caveat no longer applies to any entry. The
  picker's region groups (`Uganda — Official / Global`, `Uganda — Official
  / Africa`, `Uganda — Bantu`, `Africa`, `Europe`, `The Americas`) match
  the administrator's naming exactly.
- `TRANSLATED_LANGS` (derived from `LOCALES`) and `FULLY_TRANSLATED_LANGS`
  are now identical — all 9 shipped languages are complete, so the two
  lists no longer diverge.
- Database content localisation (glossary + `TranslationProvider` seam),
  currency/figure localisation (`lib/format.js`'s `CURRENCY_BY_LANG` /
  `LOCALE_TAG`) and digit transliteration already covered exactly this
  9-language set (Updates 32/33) and needed no change.
- **No backend, schema, endpoint or permission change** — a front-end scope
  change to an existing feature. `vite build` clean (74 modules).
- Recorded as `Implementation Status Update 102` (docs 02–18); cross-ref
  addenda on docs 19–25; `Implementation Status Note 79` in
  `Multi_Holdings_Limited_Software_Development_Procedures.docx`;
  `morise.docx` as **Update 59**; `01_Project Proposal`
  (LibreOffice-locked) staged as `*.PENDING_Update102.txt`. Pre-edit copies
  in `docx_backup91/`.

---

## Every non-English official language reaches 100% key parity (9 September 2026)

An administrator clarified the language-scoping request (Update 102): every
word on the Customer Storefront — apart from proper nouns such as "Morise
Holdings" and a subsidiary's own name — must change with the chosen
language, for all 9 official languages.

- Auditing `mbms/storefront/src/locales/en.js` (the 528-key complete
  catalogue) against each translated table found the same **66 keys**
  missing from **every one** of the 8 non-English languages (fr/es/pt/de/it
  in `eu.js`; sw/lg/ar in `regional.js`) — 462/528 = 87.5% coverage across
  the board. The gap was entirely features added after the original
  translation passes and never backfilled: the group-storefront /
  subsidiary-directory keys (Update 48) and the delivery-confirmation /
  Morise e-Stamp keys (Updates 94–96) existed only in English; a customer
  in any other language silently saw those specific strings fall back to
  English (by design — `t()`'s English fallback — but not the intended end
  state).
- All 66 keys are now translated into all 8 languages (528 new
  key/value pairs total), reusing the terminology each language already
  established elsewhere in the catalogue (e.g. French *succursale* / German
  *Filiale* for "branch" vs. French *filiale* / German *Tochtergesellschaft*
  for "subsidiary", matching `corp.whollyOwned`'s existing wording; Swahili
  *Kampuni* / Luganda *Kkampuni* / Arabic *الشركة* for "company", matching
  `account.companyName`). "Morise e-Stamp" and company/subsidiary proper
  nouns are kept untranslated, as the administrator specified. Every
  language now carries **528/528 keys — 100% parity** with English.
- A source scan for hard-coded JSX text bypassing the `t()` catalogue
  (`grep` across `src/pages` and `src/components` for capitalised text
  nodes and `placeholder`/`aria-label`/`title`/`alt` attributes) found
  nothing outstanding beyond the "MORISE" wordmark in the corporate header
  logo and one example person-name placeholder — both proper nouns, exempt
  by the administrator's own instruction.
- **No backend, schema, endpoint or permission change** — a locale-data-only
  change. `vite build` clean (74 modules); the dockerized `storefront`
  container was rebuilt and redeployed, and the live bundle was checked
  directly (`curl` against the built `assets/index-*.js`) to confirm the
  new strings — e.g. German *Tochtergesellschaften*, Swahili *Kampuni
  Tanzu*, Arabic *الشركات التابعة*, Italian *Segnala un problema* — actually
  ship, not just exist in source.
- Recorded as `Implementation Status Update 103` (docs 02–18); cross-ref
  addenda on docs 19–25; `Implementation Status Note 80` in
  `Multi_Holdings_Limited_Software_Development_Procedures.docx`;
  `morise.docx` as **Update 60**; `01_Project Proposal`'s `.~lock` file was
  checked again (`pgrep`/`lsof`) and confirmed still stale, so this section
  was pasted in directly as section **115**, no sidecar needed. Pre-edit
  copies in `docx_backup92/`.
...
implemented across the project's hosting. There are two hosting paths in
this project, and they needed different treatment.

**Netlify (Admin app, Customer Storefront) and Render (backend, per
`render.yaml`) already auto-provision and auto-renew free Let's Encrypt
certificates for every site they host** — true with zero configuration
since each was connected (Updates 104/105). Their `*.netlify.app` /
`*.onrender.com` URLs are HTTPS today, and a real custom domain added later
(Netlify: *Site configuration → Domain management*; Render: *service →
Settings → Custom Domains*) gets its own auto-provisioned, auto-renewed
certificate the same way — no code or certificate work required. There was
nothing to implement on this path; it was already live.

**The self-hosted `docker-compose.yml` stack had no TLS at all** —
`frontend`/`storefront`'s nginx and the `backend` container serve plain
HTTP only, fine for local development but not for running the stack on a
server reachable from the internet. That gap is what this update closes. A
new `mbms/deploy/` directory adds a public-facing, TLS-terminating reverse
proxy in front of the existing, unchanged containers:

- **`deploy/docker-compose.ssl.yml`** — an override adding two services:
  **`nginx-proxy`** (the only container binding ports 80/443 or
  terminating TLS; it reverse-proxies `frontend:80`, `storefront:80` and
  `backend:3001`) and **`certbot`** (a `certbot renew` loop every 12h, no
  host cron needed).
- **`deploy/nginx/app.conf.template`** + **`ssl-params.conf`** — one HTTPS
  server block per app (HTTP→HTTPS redirect on port 80; TLS 1.2/1.3 + HSTS
  on 443), templated by nginx's own `envsubst` startup step so no real
  domain name is hardcoded into the repo.
- **`deploy/init-letsencrypt.sh`** — the standard bootstrap for this
  pattern: a throwaway self-signed certificate first (so `nginx-proxy` has
  a file to start with), then the real Let's Encrypt certificate is
  requested over the HTTP-01 webroot challenge and nginx is reloaded onto
  it. Safe to re-run — skips any domain that already has a live
  certificate.
- **`deploy/.env.ssl.example`** — copy to `deploy/.env.ssl` (gitignored)
  and fill in three real, publicly resolvable domain names, a real e-mail
  address, and a `STAGING` toggle to test against Let's Encrypt's staging
  endpoint first (untrusted certs, no rate limit — production allows only
  5 duplicate certificates per domain per week).

To use it on a server with DNS already pointed at it: `cp
mbms/deploy/.env.ssl.example mbms/deploy/.env.ssl`, fill it in, then run
`mbms/deploy/init-letsencrypt.sh`. The rendered nginx config was verified
with `nginx -t` inside the pinned `nginx:1.27-alpine` image before this was
committed. **No application, schema, endpoint or permission change** — this
is infrastructure only. Recorded as `Implementation Status Update 106`
(docs 02–18); cross-ref addenda on docs 19–25; `Implementation Status Note
83` in `Multi_Holdings_Limited_Software_Development_Procedures.docx`;
`morise.docx` as **Update 63**; `01_Project Proposal` applied directly
(lock re-confirmed stale). Pre-edit copies in `docx_backup95/`.
...
### Running it fully dockerized

As of 9 September 2026, the backend, Admin app and Customer Storefront are
each containerized too (`mbms/backend/Dockerfile`, `mbms/frontend/Dockerfile`,
`mbms/storefront/Dockerfile`), alongside the Postgres/Redis containers that
were already there. One command brings up the whole stack on the same ports
as local dev:

```bash
cd mbms
docker compose up -d --build
#   Backend API      http://localhost:3001/api/v1  (Swagger at /api/v1/docs)
#   Admin app        http://localhost:5173
#   Customer Storefront  http://localhost:5174

# First run only — apply migrations and seed demo data from the host,
# against the same Postgres this stack uses:
cd backend && npx prisma migrate deploy && npx prisma db seed
```

Each frontend image is a two-stage build — `npm run build` (Vite) into a
static bundle, served by a tiny nginx that reverse-proxies `/api/*` to the
`backend` container by its Compose service name, exactly mirroring what
`vite.config.js`'s dev-server proxy does locally, so no CORS configuration
is needed either way. The backend image builds on `node:20-bookworm-slim`
(not Alpine) for both stages — Prisma's query-engine binary is glibc-linked,
and mixing an Alpine build stage with a glibc runtime (or vice versa) is a
common source of "Query Engine not found" failures; using one base for both
stages sidesteps it. See `mbms/README.md`'s "Running it — Dockerized"
section for the full walkthrough, image-layer rationale and troubleshooting.
...
The document set is **25 numbered documents** plus `morise.docx` and
...
22 §8.5). **23** (*Milling ERP & Integrated Business Management System —
Feature Specification*, new 3 September 2026, owning entity **Morise
Milling Ltd**, restating the client attachment
`Milling_Company_Software_Development_Features.docx`) is a subsidiary-system
spec, not built as a system; that subsidiary's organisation is seeded in
MBMS and, as of 8 September 2026, so is its first **24-SKU product
catalogue** (§2.2/§2.5–2.6-aligned batch tracking + min/max levels — doc 23
gains a full addendum recording it). **24**
(*Social Media Publishing — Composer, Publishing Engine & Status Dashboard —
Feature Specification*, new 3 September 2026) restates an administrator's
architecture plan for a mini Buffer / Hootsuite and records the as-built /
deferred split — the composer, fan-out engine, connections, scheduling and
dashboard are **built in MBMS**; the live platform integration (real OAuth,
platform APIs, a job queue, media processing) is **simulated**. **25**
(*Research & Consultancy Management Information System (RC-MIS) — Feature
Specification*, new 9 September 2026, owning entity **Morise Research &
Consultancy Ltd**, restating the client attachment
`Research_and_Consultancy_Software_Development_Features.docx`) is a
subsidiary-system spec, not built as a system; that subsidiary's
organisation is seeded in MBMS and, as of 9 September 2026, so is its
first **23-service catalogue** (six categories, every line priced and
storefront-visible — doc 25 gains a full addendum recording it). The
numbered docs are maintained with an **append-only status
log**: each significant change adds a dated `Implementation Status Update
(DATE)` heading rather than rewriting the body. **Seventy-six** 27 August – 9 September
2026 changes are recorded that way across docs 01–18 and `morise.docx`
(docs 19–25 each carry one-line or full addenda recording the relevant
cross-cutting and subsidiary-specific passes, but none carries the full
status log):
...
67. `… — Staff Identification Card — Back Face Sized to the Front, Blood Group Removed` (Update 92; Update 49 in `morise.docx`) — two adjustments to the Update 91 card back at an administrator's request. **Blood group removed** — the card-held **`blood_group`** column is **dropped** (migration `20260903160000_staff_id_card_drop_blood_group`; `back_notes` kept); the **`back`** object (`GET /api/v1/staff-id-cards*`, `GET /api/v1/my-hr/id-card`) no longer carries `bloodGroup` and `POST` / `PATCH /api/v1/staff-id-cards/:id` no longer accept it; the admin **Edit** dialog drops the *Blood group* field. The back still shows national ID + emergency contact (from the `Employee`), conditions of use, the "if found, return to" block, an optional note and the barcode. **Back sized to the front** — `IdCard.jsx` renders both faces at the **same width and height** (a fixed `CARD_W × CARD_H` box, overflow clipped; the back's barcode / signature lines anchored to the bottom edge). **No new endpoint or permission**; `openapi.json` unchanged (**367 paths / 467 operations**). Verified end to end (migration drops the column; `bloodGroup` gone from the card and `back` payloads; `backNotes` still round-trips; both faces the same size). `nest build` + admin `vite build` + `prisma migrate deploy` clean. Addendum on docs 19–21 and `Implementation Status Note 69` in `Multi_Holdings_Limited_Software_Development_Procedures.docx`; `01_Project Proposal` staged as `*.PENDING_Update92.txt` (LibreOffice-locked). Pre-edit copies in `docx_backup81/`.
68. `… — Morise Milling Ltd Subsidiary + Milling ERP Specification (New Doc 23)` (Update 93; Update 50 in `morise.docx`) — a **sixth wholly-owned subsidiary**, **Morise Milling Ltd** (grain milling), is seeded under Morise Holdings Limited (`prisma/seed.ts` only): company `a1000000-…-000000000007`, **six mill sites / branches** (Kampala Head Office & Central Mill, Jinja Wheat & Maize Mill, Masindi Maize Mill, Mbale Millet & Sorghum Mill, Lira Grain Mill & Silos, Kampala Distribution Depot), **twelve departments** — one per architecture module (Grain Procurement & Weighbridge; Warehouse & Inventory; Production & Milling Lines; Machine & Maintenance; Quality Assurance & Control; Packaging; Sales & Customer Management; Distribution & Delivery; Finance & Accounting; Human Resources & Workforce; Procurement & Supplier Management; IT, IoT & Integrations — ids `d1…0060`–`0071`, because `…0040`–`0042` are Morise Agro Ltd's) — and **six starter policies** (Weighbridge Ticket Integrity, Grain Acceptance & Moisture Limits, Milling Yield & Loss Tolerance, Batch Traceability End to End, Preventive Maintenance Compliance, Food Safety & Product Release). **No code, schema or permission change**; the MD's subsidiary create / update / delete / activate authority was delivered earlier. The **milling ERP is a specification, not built**: **new `docx/23` — Milling ERP & Integrated Business Management System — Feature Specification** restates the client attachment `Milling_Company_Software_Development_Features.docx` in the house format (twelve feature modules from grain procurement over a weighbridge through production, QA/QC, packaging, sales, distribution, finance and workforce to a management dashboard with IoT / PLC integration and end-to-end batch traceability; non-functional requirements; architecture; integration table; fifteen roles; technology-stack table; organisation as seeded; a five-phase roadmap; and a reuse-of-MBMS section). Storefront: `SUBSIDIARY_META` entry, a new `subsidiary-milling.svg` brand banner (mirrored into both apps) and a News item. Group figures become **6 subsidiaries / 26 sites / branches**. Verified — the `/organization` API returns the company, 6 branches, 12 departments and 6 policies; admin + storefront `vite build` clean. Addendum on docs 19–22 and `Implementation Status Note 70` in `Multi_Holdings_Limited_Software_Development_Procedures.docx`; `01_Project Proposal` staged as `*.PENDING_Update93.txt` (LibreOffice-locked). Pre-edit copies in `docx_backup82/`.
69. `… — Automatic Morise e-Stamp on a Customer-Acknowledged Delivery` (Update 94; Update 51 in `morise.docx`) — **implements** the spec-only Update 66 / doc 22 addendum. New **`POST /api/v1/customer-portal/orders/:id/delivery/acknowledge`** (customer JWT; `condition` = `good` / `damaged` / `incomplete` / `not_received` + optional `note`), allowed only once and only after the order's delivery is `delivered`. **`good`** → one transaction stamps the order's invoice — **`invoices.stamped_at`** / unique **`stamp_number`** `MOR-ESTAMP-YYYY-NNNNNN` / **`stamp_condition`** (new nullable columns, migration `20260903180000_invoice_estamp_delivery_ack`) — logs a delivery event + an `invoice.estamp.issued` audit + a best-effort staff notification. **Anything else** → **`deliveries.exception_opened_at`** set, the report logged, a `delivery.exception_opened` audit + notification, **no stamp**. The delivery also records `customer_ack_condition` / `customer_ack_at` / `customer_ack_note` / `customer_ack_by`. Surfaced on the customer delivery-tracking view (`canAcknowledge`, ack fields, `eStamp`), the invoice list / detail (`eStamped` / `stampNumber`), the staff `DeliveryService.get`, and the **invoice PDF** — which now draws a **navy / amber Morise seal** (Morise "M" mark + `MORISE e-STAMP` + stamp number) top-right. Storefront: a **"Did everything arrive in good condition?"** panel on the *My Orders* detail (confirm receipt / report a problem) and a **"Morise e-Stamp"** badge on the *Invoices* list. Seed adds one un-acknowledged completed delivery (`DEL-2026-0001`) for the demo. **No new permission.** Verified end to end (a `good` ack issued `MOR-ESTAMP-2026-000001` + PDF seal; `not delivered` → 409, `already acknowledged` → 409, `no delivery` → 404; `damaged` → exception opened, no stamp). `nest build` + storefront `vite build` + `prisma migrate deploy` clean; `openapi.json` regenerated (**368 paths / 468 operations**). Supersedes the "spec only — nothing built" status of item 41 / Update 66. A **built** addendum on `docx/22` + cross-ref addenda on docs 19–21 and 23; `Implementation Status Note 71` in `Multi_Holdings_Limited_Software_Development_Procedures.docx`; `01_Project Proposal` staged as `*.PENDING_Update94.txt` (LibreOffice-locked). Pre-edit copies in `docx_backup83/`.
70. `… — A Clear "Confirm Receipt" Button / Link for the Customer e-Stamp Confirmation` (Update 95; Update 52 in `morise.docx`) — **usability follow-up to Update 94**: the customer confirmation that triggers the Morise e-Stamp is surfaced as a first-class call to action. New read **`GET /api/v1/customer-portal/orders/awaiting-confirmation`** (customer JWT) → the caller's orders with a completed, unconfirmed delivery; `GET /customer-portal/dashboard-summary` gains **`deliveriesToConfirm`** / **`firstDeliveryToConfirmOrderId`**. Storefront: a dedicated **`/orders/:id/confirm`** page (order + delivery summary, a prominent **"Confirm — received in good condition"** button, a report-a-problem form, an e-Stamp / exception outcome + a **"Download stamped invoice"** link), a **"Confirm receipt →"** link on each pending order row in *My Orders*, and a **Home prompt card**. The Update 94 acknowledge endpoint, e-Stamp and in-detail panel are **unchanged**; **no schema change, no new permission**. Verified end to end (`awaiting-confirmation` + dashboard count 1 → 2 → 1). `nest build` + storefront `vite build` clean; `openapi.json` regenerated (**369 paths / 469 operations**). Addendum on `docx/22` + cross-ref addenda on docs 19–21 and 23; `Implementation Status Note 72` in `Multi_Holdings_Limited_Software_Development_Procedures.docx`; `01_Project Proposal` staged as `*.PENDING_Update95.txt` (LibreOffice-locked). Pre-edit copies in `docx_backup84/`.
71. `… — Confirm-Receipt Button Driven by the Order's Delivered Status` (Update 96; Update 53 in `morise.docx`) — **refinement of Updates 94–95**: the customer's Confirm-receipt button is driven by the **order's status** set by admin staff, and works for **any delivered order** (a Morise Logistics delivery record is optional). `Order` gains **`delivered_at`** + **`customer_receipt_confirmed_at`** / **`_condition`** / **`_note`** (migration `20260903200000_order_customer_receipt`, which back-fills `delivered_at`); `advanceStatus` and the delivery-completion path stamp `delivered_at` on reaching `delivered`. `GET /customer-portal/orders` + `/:id` gain `deliveredAt` / the receipt fields / **`canConfirmReceipt`** + `eStamped` / `stampNumber` on the invoice. `POST .../delivery/acknowledge` is **order-gated** (order `delivered`, once only, no delivery record needed): **`good`** stamps the order's invoice + mirrors onto a delivery record if present, else records a **dispute** (`order.receipt.disputed` audit). `awaiting-confirmation` + the dashboard `deliveriesToConfirm` count are **order-driven**. Storefront panels / `/orders/:id/confirm` / the row link re-pointed at the order. **No new endpoint or permission**; `openapi.json` unchanged (**369 paths / 469 operations**). Verified end to end (staff advance → delivered; customer `damaged` → dispute no stamp, `good` → e-Stamp + mirror; guards 409). `nest build` + storefront `vite build` + `prisma migrate deploy` clean. Addendum on `docx/22` + cross-ref addenda on docs 19–21 and 23; `Implementation Status Note 73` in `Multi_Holdings_Limited_Software_Development_Procedures.docx`; `01_Project Proposal` staged as `*.PENDING_Update96.txt` (LibreOffice-locked). Pre-edit copies in `docx_backup85/`.
72. `… — Social Media Publishing — Composer, Publishing Engine & Status Dashboard (New Doc 24)` (Update 97; Update 54 in `morise.docx`) — a **mini Buffer / Hootsuite** inside MBMS. New module **`src/social-publishing`** at **`/api/v1/social`**. Schema (migration `20260903220000_social_publishing`): **`social_connections`** (one simulated OAuth connection per platform — status, account label, ~60-day token expiry, placeholder token reference), **`social_posts`** (master body, media type + text-reference URL, link, status, `scheduledFor`) and **`social_post_targets`** (one row per platform per post — caption override, status, external id + permalink, error, attempts). `PLATFORM_RULES` carries each platform's character limit / media requirement / truncation behaviour (facebook, x, instagram, linkedin, youtube, tiktok, whatsapp, telegram, other) and `GET /social/posts/:id` returns a live per-target preview (effective caption, char count vs limit, over-limit / will-truncate, media-rule + connection warnings). Endpoints: `GET /social/platforms`; `POST /social/platforms/:platform/{connect,disconnect}`; `GET`/`POST /social/posts`, `GET`/`PATCH`/`DELETE /social/posts/:id`; **`POST /social/posts/:id/publish`** (the single button), `POST /social/posts/:id/retry`, `POST /social/run-scheduled` (cron stand-in); `GET /social/dashboard`. Three new **governance** permissions — **`social.post.manage`** (draft / edit / delete, connect / disconnect), **`social.post.publish`** (publish / retry / run scheduled), **`social.post.viewAll`** (composer / posts / dashboard) — the *"can create drafts vs can publish"* split; manage+publish+viewAll → Super Administrator / Managing Director / IT Administrator, Sales Manager gets manage+viewAll, oversight roles get viewAll. Admin gains a **Social Publishing** screen under ADMIN / BACKEND (four tabs). The publish itself is **SIMULATED** — no OAuth handshake, platform API call or job queue (the same documented deferral as Kafka / object storage / email / SMS); the connection / post / target / partial-failure / retry / dashboard shape is real. Verified end to end (9 platforms, 4 connected; Instagram missing media → `failed` → post `partially_failed`; add media + retry → `published`; disconnect → target fails; `run-scheduled` fired a due post; Sales Manager `POST` → 403, Auditor `GET` 200 / `POST` 403). `nest build` + admin `vite build` + `prisma migrate deploy` clean; `openapi.json` regenerated (**378 paths / 481 operations / 188 schemas**). **New `docx/24`** restates the administrator's architecture plan and records the as-built / deferred split; `Implementation Status Update 97` on docs 02–18; cross-ref addenda on docs 19–23; `Implementation Status Note 74` in `Multi_Holdings_Limited_Software_Development_Procedures.docx`; `01_Project Proposal` staged as `*.PENDING_Update97.txt` (LibreOffice-locked). Pre-edit copies in `docx_backup86/`.
73. `… — Product Catalogue for Morise Milling Ltd + Branch-Tagged Product Registration (Doc 23)` (Update 98; Update 55 in `morise.docx`) — an administrator's first product catalogue for **Morise Milling Ltd** (Update 93, zero products until now). `prisma/seed.ts` gains **24 SKUs across 7 categories**: Maize Milling Products (5), Wheat Milling Products (3), Millet & Sorghum Products (3), By-Products & Animal Feed (4), Raw Grain — Intake (3, no `unitPrice`, internal only), Packaging Materials (2, no `unitPrice`, internal only) and Milling Services (4), each attributed to its producing mill/depot, with `trackBatches` + min/max stock levels per docx/23 §2.2 and §2.5–2.6. Product registration is generalised group-wide: `CreateProductDto`/`UpdateProductDto` gain an optional **`branchId`** (the field has existed on `Product` since 29 August 2026, but only the seed could set it), validated by a new `ProductsService.assertReferencesBelongTo` against the product's own company, and every product read gains `companyName`/`branchName`. The **Admin Products screen is rebuilt**: a subsidiary filter, Company/Branch/Stock columns, a per-row **Edit** action, the New Product form's company-UUID text field replaced by linked Company → Branch → Category dropdowns, and a new **Edit Product** modal wired to `PATCH /products/:id` (live since Sprint 4, no screen before). **No new endpoint, migration or permission**; `openapi.json` regenerated **unchanged** (**378 paths / 481 operations / 188 schemas**). Verified end to end — reseeded, all 24 rows confirmed in Postgres with correct branch attribution; `nest build` clean. Group catalogue → **58 products / 25 categories** (53 storefront-priced, 5 internal-only); **all six operating subsidiaries now sell at least one product or service**. Doc 23 gains a full **"Product Catalogue Seeded for Morise Milling Ltd"** addendum + cross-ref addenda on docs 19–22; `Implementation Status Note 75` in `Multi_Holdings_Limited_Software_Development_Procedures.docx`; `01_Project Proposal` staged as `*.PENDING_Update98.txt` (LibreOffice-locked). Pre-edit copies in `docx_backup87/`.
74. `… — Morise Research & Consultancy Ltd — Seventh Subsidiary + RC-MIS Specification (New Doc 25)` (Update 99; Update 56 in `morise.docx`) — an administrator asked to add a subsidiary for research and other types of consultancy, based on the client attachment `Research_and_Consultancy_Software_Development_Features.docx`. **Morise Research & Consultancy Ltd** seeded as the group's **seventh wholly-owned (100%) subsidiary** (`a1…008`, Kololo, Kampala) the same low-footprint way as every subsidiary before its first feature build: **5 field offices** (`b1…0050`–`0054` — Kampala HQ, Gulu, Mbale, Mbarara, a Karamoja field station), **12 departments** (`d1…0080`–`0091`, condensing the client's 14-node RC-MIS architecture), **6 starter policies** (`p1…0050`–`0055` — Research Ethics & Informed Consent, Data Protection & Participant Confidentiality, Fieldwork Safety & Security, Data Quality Assurance, Consultant & Enumerator Conflict-of-Interest, Contract/Grant/Proposal Compliance). **seed.ts only** — no code, schema or permission change; **not a storefront seller** (0 products — consultancy is billed on contracts/timesheets, not a catalogue). **New `docx/25`** restates the client's 14 feature modules (CRM, Proposals & Tenders, Contracts & Grants, Research Projects, Field Data Collection & Sampling, Data Quality/Management/Analysis, Qualitative Research, M&E, Reports/Deliverables/Knowledge, Ethics/Risk/Compliance, Consultants/HR, Finance/Timesheets/Invoicing, Client Portal, Management Analytics) in house format with NFRs, architecture, integration points, 14 user roles, tech stack, the organisation as built, a 5-phase roadmap and platform-reuse mapping. `corporate.js` gains a `SUBSIDIARY_META`/`SUBSIDIARY_BANNER`/News entry; new **indigo-accent** `brand/banners/subsidiary-research-consultancy.svg` (bar-chart + magnifying-glass motif) mirrored into both apps and added to `brand/README.md`. Verified end to end — company/5 branches/12 departments/6 policies confirmed in Postgres + the Admin organisation API; public overview lists it; both banner assets resolve live. **No new endpoint, migration or permission**; `openapi.json` unchanged. Group → **7 subsidiaries, 31 sites/branches**. Cross-ref addenda on docs 19–24; `Implementation Status Note 76` in `Multi_Holdings_Limited_Software_Development_Procedures.docx`; `01_Project Proposal` staged as `*.PENDING_Update99.txt` (LibreOffice-locked). Pre-edit copies in `docx_backup88/`.
75. `… — Services Catalogue for Morise Research & Consultancy Ltd (Doc 25)` (Update 100; Update 57 in `morise.docx`) — an administrator's first products pass for **Morise Research & Consultancy Ltd** (Update 99, zero products until now, seeded on the reasoning that a consultancy is billed on contracts/timesheets). Following the **Morise Collateral Management Ltd** precedent (contract-billed but carrying a priced catalogue since Update 60), `prisma/seed.ts` gains **23 services (`RC-7001`–`RC-7051`) across 6 categories** — Research & Evaluation Studies (6), Field Data Collection (4), Data Management & Analysis (4), Monitoring & Evaluation Services (3), Technical & Policy Consultancy (4), Training & Capacity Building (2) — reusing the existing `ensureService()`/`ensureCategory()` helpers already exercised for the Collateral Management catalogue. Every line is `service` with a **`unitPrice`** (UGX, per study/session/dataset/day), so the **entire catalogue is storefront-visible**; study design/analysis/policy-advisory/day-rate sit at the Kampala Head Office, fieldwork-heavy lines at the Gulu/Mbale/Mbarara/Karamoja field offices. **Seed-only** — no schema, code, endpoint or permission change. Verified end to end — 23 rows confirmed in Postgres with correct branch/category attribution; the Admin Products screen filtered to the subsidiary screenshot-verified with no console errors; `nest build` clean; `openapi.json` regenerated **byte-identical** (**378 paths / 481 operations / 188 schemas**). Group catalogue → **81 products / 31 categories** (76 storefront-priced, 5 internal-only); **all seven operating subsidiaries now sell at least one product or service**. Doc 25 gains a full **"Services Catalogue Seeded for Morise Research & Consultancy Ltd"** addendum + cross-ref addenda on docs 19–24; `Implementation Status Note 77` in `Multi_Holdings_Limited_Software_Development_Procedures.docx`; `01_Project Proposal` staged as `*.PENDING_Update100.txt` (LibreOffice-locked). Pre-edit copies in `docx_backup89/`.
76. `… — Dockerized Backend, Admin App and Customer Storefront` (Update 101; Update 58 in `morise.docx`) — an administrator asked to dockerize the backend, Admin app and Customer Storefront (until now only Postgres/Redis ran as containers). **`backend/Dockerfile`** — two-stage build on **`node:20-bookworm-slim`** for both stages (deliberately not Alpine, to avoid a musl/glibc Prisma query-engine mismatch): `npm ci` → `prisma generate` → `npm run build` (`nest build`, preserving the Swagger CLI plugin's OpenAPI schema generation); the runtime stage ships only compiled `dist/`, the generated Prisma Client and production `node_modules`; both stages install `openssl`/`ca-certificates` (Prisma's documented fix for a libssl-detection warning on `-slim`). **`frontend/Dockerfile`** and **`storefront/Dockerfile`** — matching two-stage builds on `node:20-alpine` → `nginx:1.27-alpine`, each with an `nginx.conf` reverse-proxying `/api/*` to the `backend` container by Compose service name (mirrors each app's `vite.config.js` dev-server proxy — no CORS change needed). **`docker-compose.yml`** gains `backend`/`frontend`/`storefront` services (`backend` waits on a new Postgres healthcheck; `frontend`/`storefront` keep the same host ports, 5173/5174, their Vite dev servers already used). Migrations/seeding stay a **host-run step** (`npx prisma migrate deploy && npx prisma db seed`) — unchanged from before the backend itself was containerized, since the runtime image omits the `prisma` CLI and `ts-node` to stay small. **No schema, endpoint or permission change**; `openapi.json` unchanged. Verified end to end — all three images build clean; `docker compose up -d --build` brings up all five containers healthy; existing seeded data (8 companies, 81 products) survived a Postgres container recreate (the `mbms_pg_data` named volume, not the container, holds it); a staff login through the dockerized Admin app and a page load of the dockerized Customer Storefront both succeeded via the nginx-to-backend proxy with **zero browser console errors** (screenshot-verified). Cross-ref addenda on docs 19–25; `Implementation Status Note 78` in `Multi_Holdings_Limited_Software_Development_Procedures.docx`; `01_Project Proposal` staged as `*.PENDING_Update101.txt` (LibreOffice-locked); `docx/How to Run the System (Local Development).docx` gains a new "Running it fully dockerized" section. Pre-edit copies in `docx_backup90/`.
...
81. `… — SSL/TLS Certificates via Let's Encrypt` (Update 106; Update 63 in `morise.docx`) — an administrator asked for SSL certificates to be added, updated and implemented across the project's hosting. Netlify (Admin app, Customer Storefront) and Render (backend, per `render.yaml`) already auto-provision and auto-renew free Let's Encrypt certificates for every site they host, including any custom domain added later — nothing to implement there. The gap was the self-hosted `docker-compose.yml` stack, which served plain HTTP only. New `mbms/deploy/` directory: `docker-compose.ssl.yml` adds a public-facing `nginx-proxy` service (the only container terminating TLS; reverse-proxies to the existing, unchanged `frontend`/`storefront`/`backend` containers) and a `certbot` service (renewal loop every 12h); `nginx/app.conf.template` + `ssl-params.conf` (HTTP→HTTPS redirect, TLS 1.2/1.3 + HSTS, one server block per app, domain names templated via `envsubst` — none hardcoded); `init-letsencrypt.sh` bootstraps a dummy certificate so nginx can start, then requests the real certificate over the HTTP-01 webroot challenge and reloads nginx (idempotent — skips domains that already have a live certificate); `.env.ssl.example` for the three real domains, an e-mail address and a Let's-Encrypt-staging toggle. `.gitignore` gained entries for the runtime certificate directory and the real `.env.ssl`. **Verified**: the rendered nginx config passed `nginx -t` inside the pinned `nginx:1.27-alpine` image. **No application, schema, endpoint or permission change** — infrastructure only. Cross-ref addenda on docs 19–25; `Implementation Status Note 83` in `Multi_Holdings_Limited_Software_Development_Procedures.docx`; `01_Project Proposal` applied directly (lock re-confirmed stale). Pre-edit copies in `docx_backup95/`.
...
- **Admin app:** all previously working screens intact, under 4 collapsible categories; **Users & Settings** only offers grants the signed-in admin is authorised to make; the **sidebar and routes are permission-filtered**. **Companies & Company detail** gain Activate/Deactivate/Delete controls for subsidiaries and their branches / departments / policies (MD / Super Admin / IT Admin). The **Inventory** screen is now live — company-level stock on hand, re-order points, receipt / issue / count adjustments and an immutable movement ledger (view with `product.viewAll`, adjust with `product.manage`). **Marketing & Promos** (discount codes + promo banners) and **CMS / Site Builder** (draft/publish content pages, the group's **social media channels**, the **landing-page FAQ**, and the **newsletter sign-ups** list) are also live, gated by `marketing.*` / `cms.*` permissions — held in full (`manage` + `viewAll`) by the **Managing Director**, Super Administrator and IT Administrator. **Social Publishing** (a mini Buffer / Hootsuite — one master post fanned out to many social channels in one click, per-channel caption overrides, publish now or schedule, per-platform status dashboard; the live platform integration is simulated) is live under ADMIN / BACKEND, gated by the new `social.post.manage` / `social.post.publish` / `social.post.viewAll` governance permissions with a drafts-vs-publish split. The **HR Dashboard** (read-only headcount / attendance / recruitment / upcoming-events roll-up) and **Departments** (list-across-companies with headcount, create / rename / delete) screens are live under Human Resources, joined by **Staff ID Cards** — an **automatic front-and-back staff identification card** for every active employee (issued on employee creation, back-filled by a bulk sweep, revoked / restored with employment status; a **front** with photo / QR / details and a **back** (same size as the front) with conditions of use, an "if found" return-address block, national ID / emergency contact and a barcode; generate / issue / edit / reissue / revoke / restore), gated by `employee.idcard.manage` / `employee.idcard.viewAll`, with a **My HR → My ID Card** self-service view. **Shift Scheduling** (weekly roster builder, publish + notify, coverage & clash view — reuses `attendance.*`) and **Payroll** (monthly run with Uganda PAYE/NSSF, payslips, salary advances, balanced GL posting — new `payroll.*` permissions with a preparer/approver split) complete that section. The **MY HR** self-service set — Clock In/Out, My Leave, My Shifts, My Performance, My Payslips, My ID Card, My Salary Advances — is live for any login **linked to an employee record** (`src/my-hr/`, read projection + a self-service salary-advance request; no new permission). The **Financial & Accounting** overview (`/finance`) is a read-only landing screen — consolidated group position, working-capital "needs attention" figures, period-close status, and shortcut cards into the sub-screens (`src/finance/`; any finance-area permission; no new permission). **Every entry in all four navigation categories now opens a working screen.**
- **Storefront:** the **public entry is a holding-company mini-site** — a signed-out visitor gets a corporate site (landing with live group figures, Companies directory + per-company pages, Profile, Group Leadership, Group Overview, News, Contacts), all readable without an account and fed by a new public `GET /customer-portal/public/overview`; a signed-in customer still lands on their dashboard. These pages use the **same design system as the shop** (navy/white/amber, system fonts) so the public site and the storefront look like one product. Beyond that, customers **register / sign in with email, phone, account number or Google and reset a forgotten password** themselves. The shop is a **group storefront** — the priced goods **and services of all seven Morise subsidiaries** (Agro inputs & fuel; Logistics road freight & delivery; Clearing & Forwarding customs & freight-forwarding; Warehouse Management storage & handling; Collateral Management CMA setup, field-warehousing control, inspection, valuation & coverage monitoring, release authorisation, warehouse-receipt issuance; Milling flour, by-products & animal feed, and grain/milling services; **Research & Consultancy studies, field data collection, data analysis, M&E and technical/policy consultancy**) in one catalogue of 76 items, each listing **tagged with its subsidiary and fulfilling branch**, filterable by company or branch, with a **Subsidiaries directory** page; the customer's own **subsidiary and branch** are shown in the header and on My Account. A basket spanning subsidiaries is **placed as one order per subsidiary** (each invoiced and GL-posted in that company's books). Customers can also **switch the whole interface into 9 official languages** — English, Kiswahili, Luganda, Arabic (right-to-left), German, French, Spanish, Italian and Portuguese, grouped as Uganda — Official/Global, Uganda — Official/Africa, Uganda — Bantu, Africa, Europe and The Americas — with **every word changing, at 100% key parity (528/528) with English, except proper nouns** (Morise Holdings, subsidiary names) — including the **pre-login disclaimer gate and the corporate landing pages** (first visit picks the language from `?lang=` or the browser); **catalogue, support text, promo banners, CMS content pages, the FAQ, the disclaimer statements, order history, the Home dashboard, the account statement and the downloaded statement / invoice PDFs are shown in the chosen language**, **monetary amounts are converted and formatted for that locale** (UGX → AED / EUR / BRL, fixed demo rates), and figures/digits follow the language's script (a non-Latin-script PDF keeps its label words in English pending font embedding). Customer input — including an order cancellation note, a payment reference and a typed discount code — is stored / matched canonically (English, UGX, 0-9) with the original kept. Customers can **apply a discount code at checkout** (validated + redeemed server-side, single-subsidiary carts; discount taken before VAT; the outcome message is localised), see **promo banners** on the home page, and read **published content pages** at `/page/:slug` (About / Terms / Delivery). Once **admin staff mark an order delivered**, a **"Confirm receipt"** button / link appears on *My Orders* (and a Home prompt + a dedicated `/orders/:id/confirm` page); the customer clicks it to **confirm receipt in good condition** and MBMS then **automatically places a Morise e-Stamp on the order's invoice** (`MOR-ESTAMP-YYYY-NNNNNN` + a navy/amber seal on the invoice PDF); reporting a problem instead records a **dispute** with no stamp (`POST /customer-portal/orders/:id/delivery/acknowledge`, order-gated — a Morise Logistics delivery record is optional). The **storefront footer** (public and signed-in) shows the **group's social media channels**, managed by an admin in CMS / Site Builder and served by the public `GET /customer-portal/public/social-links`. The **landing page** carries a **newsletter sign-up band** (`POST /customer-portal/public/newsletter`, rate-limited) and an **FAQ section** whose Q&A an admin manages in CMS / Site Builder (`GET /customer-portal/public/faqs`).
- **Subsidiaries:** Morise Agro Ltd, **Morise Logistics Ltd** (Jinja Branch; four priced storefront services incl. last-mile delivery `LOG-2003`; its **Delivery Management System** specified in doc 22 — Phase 1 back end built in MBMS and **activated** (`delivery.manage` / `delivery.viewAll` seeded + granted, 31 Aug 2026); a dispatcher can create a delivery for a customer's order and assign a Morise Logistics Ltd delivery persona through `/api/v1/delivery` (verified); an Admin dispatch screen and seeded personas are still to build. **A customer can now confirm receipt of a completed delivery in good condition and MBMS automatically e-stamps the order's invoice** (`POST /api/v1/customer-portal/orders/:id/delivery/acknowledge`, Morise e-Stamp on the invoice + a seal on the PDF; a problem report opens a delivery exception — built 3 Sep 2026)), **Morise Clearing & Forwarding Ltd** (6 branches, 8 departments, 4 policies; its system specified in doc 19, not built), **Morise Warehouse Management Ltd** (5 branches, 9 departments, 5 policies; its 12-module WMS specified in doc 20, not built), and **Morise Collateral Management Ltd** (new — collateral management agent; 6 sites, 9 departments, 6 policies; **10 priced services** in the storefront (CMC-5001–CMC-5010) drawn from the client's services catalogue; its 10-module Multi-Site Collateral Management System specified in doc 21, not built), and **Morise Milling Ltd** (grain milling; 6 mill sites, 12 departments, 6 policies; **24-SKU product catalogue seeded 8 Sep 2026** — maize / wheat / millet & sorghum flour, by-products & animal feed, raw grain intake, packaging materials, and milling services, 19 of them storefront-priced; its 12-module Milling ERP & Integrated Business Management System specified in doc 23 remains a specification, not built), and **Morise Research & Consultancy Ltd** (research & consultancy services; 5 field offices, 12 departments, 6 policies; **23-service catalogue seeded 9 Sep 2026** across 6 categories — research & evaluation studies, field data collection, data management & analysis, M&E services, technical & policy consultancy, training & capacity building, all 23 storefront-priced; its 14-module RC-MIS specified in doc 25 remains a specification, not built). Group: **7 subsidiaries, 31 sites / branches, 81 catalogue items (76 storefront-priced)**.
- **Docs:** **25 numbered documents** + `morise.docx`; docs 01–18 and `morise.docx` carry the seventy-seven 27 August – 9 September 2026 status updates (navigation redesign; delegated administration; permission-based visibility; storefront localisation ×3; database content localisation; currency & figure localisation; subsidiary lifecycle; warehouse-management subsidiary; inventory section activated; marketing & promos and CMS / site builder activated; MD full control of marketing & CMS; localisation extended to promo banners & CMS content; HR Dashboard & Departments activated; Shift Scheduling & Payroll activated; My HR self-service activated; demo staff logins linked to employee records; Financial & Accounting overview activated; customer sign-up, multi-identifier login & password reset; one storefront across every subsidiary and its branches; Clearing / Forwarding & Warehouse-Management catalogues added to the storefront; content localisation extended to the group storefront (read in chosen language, store input in English); OpenAPI / Swagger API documentation generated from the backend; corporate landing page & public holding-company mini-site; corporate site restyled to match the shop page; public content pages folded into the corporate site chrome; admin-managed social media channels shown in the storefront footer; landing-page newsletter sign-up + admin-managed FAQ section; content localisation completed across orders & payments; content localisation final edges — dashboard, account statement & exported PDFs, discount-code input; Collateral Management subsidiary + new doc 21; full-page demonstration-disclaimer gate with admin-managed statements; disclaimer gate & corporate landing pages localised into the chosen language; Collateral Management priced services catalogue added to the storefront; Managing Director full operational authority across every admin category; Morise brand mark / professional logo across both apps; Morise banner family across both apps; brand banners displayed across the customer storefront; Delivery Management System specification for Morise Logistics Ltd (new doc 22); automatic Morise-stamped invoice on customer-acknowledged delivery (doc 22 addendum); admin assignment of a Morise Logistics Ltd delivery persona to an order (doc 22 addendum); delivery module activated — `delivery.manage` / `delivery.viewAll` seeded + granted; Customer Storefront wording — Services Catalog / Add to Invoice / Invoice; Payroll Management feature specification (core built Update 41; structure / bank-schedule / reporting suite recorded as backlog); Payroll: salary structures, allowances, overtime & bonuses built (Update 71); Accounting & Finance feature section recorded (Update 72 — already built; one gap: dedicated AR reconciliation); Cash & Bank Management feature section recorded (Update 73 — reconciliation + cash position built; register / petty cash / vouchers / transfers / forecasting on the backlog); Budget Management feature section recorded (Update 74 — company / project budgets + budget-vs-actual built; dimensions / approval / revisions / utilization / alerts on the backlog); Sales Management feature section recorded (Update 75 — customer management + order-to-cash core built; quotations / returns / commissions / targets and the whole CRM on the backlog); Procurement & Purchasing feature section recorded (Update 76 — supplier master + AP invoice tail built; requisitions / POs / RFQs / quotations / GRNs on the backlog); Supplier Management feature section recorded (Update 77 — supplier master built; multi-contact / contract & document files / suspension / evaluation & performance on the backlog); Supplier Management implemented (Update 78 — multi-contact list, time-bound suspension, evaluation scorecards & a derived performance summary built; contract / document files still on the backlog); Inventory Management implemented (Update 79 — warehouses, stock locations, per-warehouse balances, transfers, returns, min / max levels, batch / expiry & serial tracking and the eight inventory reports built on top of the 28 Aug company-level section; put-away to a location, FEFO consumption & stock reservations on the backlog); Warehouse Management operations layer implemented (Update 80 — warehouse staff by role, receiving [goods receipts], picking / packing / dispatch [pick lists tied to sales orders], warehouse stock counts + reconciliation, functional bins and barcode lookup built on top; put-away to a bin, wave picking, pack-station label printing & ASN scheduling on the backlog); Asset Management implemented (Update 81 — first-class asset categories with depreciation defaults, supplier / PO / warranty on the asset, periodic condition inspections, first-class insurance policies, an append-only asset-history log and four asset reports built on top of the Sprint-11 Asset Service; an asset-documents file store, reducing-balance depreciation, scheduled runs & revaluation on the backlog); Fleet & Vehicle Management module built (Update 82 — a new module: vehicle register & ownership, driver assignment, odometer / mileage history, fuel management, service schedules & repairs, statutory renewals, accident records, an auto-fed vehicle-expense log and five reports incl. fuel consumption [L/100km, cost/km]; new fleet.manage / fleet.viewAll permission; vehicle insurance & inspection reuse the linked asset; live GPS telematics on the backlog); **remaining feature catalogue sections 17–57 recorded against the build (Update 83 — Project/Expense/Support/Audit-Trail/Inter-Company/Consolidated/Multi-Branch/BI/Security/API/workflows/Performance built; Contract/Investment/Shareholding/Tax/Multi-Currency/Approval-engine/Communication/Marketing/Search/Import-Export/Mobile/Compliance partial; Legal/Document-store/Loans/Board/Audit-Management/Backup-DR/Integrations/BPM-designer/Exit planned; architecture core already matches, distributed stack deferred) + Notifications & Alerts implemented (a new src/alerts cross-module feed at /alerts + /alerts/summary; new Alerts screen; no schema, no new permission); **Final System Objective answered (Update 84 — a new src/executive module: /executive/questions + /executive/overview answer the spec's seventeen group-management questions from live data, scoped per BR-01 [Group role → whole group, subsidiary-scoped → their companies]; 15 answered, contract-expiry & department-spend partial, investments on the roadmap; new Executive Q&A screen; read-only, no schema, no new permission — completes the documentation pass over the master specification)**; **payroll payslips visible to staff on approval (Update 85 — `GET /api/v1/my-hr/payslips` widened from `paid` to `approved`-or-`paid` so staff see their payslip in My HR → My Payslips as soon as the run is approved, marked "awaiting payment" until paid; approve now notifies each member with a login; scoping unchanged; read-only, no schema, no migration, no new permission)**; **alerts on the admin bell, routed by department (Update 86 — the Update 83 Alerts feed is mirrored onto the bell as `alert:<category>` notifications by `AlertsService.syncToNotifications`, reconciled on each `/notifications` read, no scheduler; every alert tagged with its owning department by keyword-matching the company's department names; a staff member sees their department's alerts + company-wide ones, a group-visibility / no-department user sees the whole scope; `Notification.department_id` added, migration `20260902140000_notification_department`; no new permission or endpoint)**; **alert notifications made visible on the bell (Update 87 — the Update 86 rows were rendering white-on-white inside the navy `.topbar`; the dropdown now sets `color: var(--text)` + opaque white background; routing made deterministic — ordered department-name keywords, most-specific first, first match wins, with `field` / `operations` catch-alls; no schema / endpoint / permission change)**; **social media channels managed from one shared record (Update 88 — a new single-row `social_content` table [handle / display name / tagline / visibility] drives every storefront social channel from CMS / Site Builder: `GET`/`PUT /api/v1/cms/social-content`, `POST /api/v1/cms/social-content/apply` [one click pushes the record to every channel; `createMissing` also adds a channel per platform, `overwriteUrls` rebuilds each URL from the handle] and `DELETE /api/v1/cms/social-content/channels` [one click deletes them all]; existing `cms.manage` / `cms.viewAll`, no new permission; per-channel `social_links` table unchanged; storefront footer shows the shared tagline; migration `20260902150000_social_content`)**; **every social media platform managed from one CMS roster (Update 89 — the CMS *Social media channels* panel is now a fixed roster of all nine supported platforms, each an editable row [URL / label / visibility / order] whether or not a channel exists; `social_links` gains a `UNIQUE` constraint on `platform` [migration `20260902160000_social_link_platform_unique`, de-duplicating first], the free-form add form is replaced and `POST /cms/social-links` 409s on a duplicate platform; new `GET /api/v1/cms/social-links/roster` and `PUT /api/v1/cms/social-links/platform/:platform` [upsert / delete by platform] under existing `cms.manage` / `cms.viewAll`)**; **automatic staff identification cards for active administration staff (Update 90 — a new `staff_id_cards` table [one-to-one with `employees`; `MID-XXXXXXXX` card number, opaque verification code, `active`/`revoked`/`expired`, three-year expiry, text photo ref] + `src/staff-id-cards` module + two HR permissions `employee.idcard.manage` / `employee.idcard.viewAll`; a card is issued automatically on employee creation, `POST /api/v1/staff-id-cards/generate` back-fills anyone missing one and expires stale cards, and the card is revoked / restored automatically with employment status; admin Staff ID Cards gallery + `My HR → My ID Card` self-service; migration `20260903120000_staff_id_cards`)**; **staff identification card — front and back (Update 91 — the card gains a distinct back face: conditions of use, an "if found, return to" block from the issuing company's registered address, the holder's national id / emergency contact [derived from the `Employee`], an optional blood group + free-text note [new nullable `staff_id_cards.blood_group` / `back_notes`, migration `20260903140000_staff_id_card_back`] and a barcode of the card number; every card read returns a `back` object, `POST`/`PATCH /api/v1/staff-id-cards/:id` take `bloodGroup`/`backNotes` under the existing `employee.idcard.manage`, no new endpoint or permission; `IdCard.jsx` renders both faces — flip on the admin gallery, side by side on My ID Card)**; **staff identification card — back sized to the front, blood group removed (Update 92 — the `staff_id_cards.blood_group` column is dropped [migration `20260903160000_staff_id_card_drop_blood_group`], the `back` object and `POST`/`PATCH` no longer carry `bloodGroup`, and `IdCard.jsx` renders the front and back at one fixed `CARD_W × CARD_H` size; `back_notes` kept, no new endpoint or permission)**; **Morise Milling Ltd subsidiary + Milling ERP specification (Update 93 — a sixth wholly-owned subsidiary, Morise Milling Ltd [grain milling], seeded in `prisma/seed.ts`: company `a1…007`, 6 mill sites, 12 departments [`d1…0060`–`0071`], 6 starter policies; the milling ERP is a specification — new `docx/23` restating the client attachment `Milling_Company_Software_Development_Features.docx` [twelve modules, five-phase backlog]; storefront gets a `SUBSIDIARY_META` entry + a `subsidiary-milling.svg` brand banner; group figures → 6 subsidiaries / 26 branches; seed + docs + a brand banner only, no code / schema / permission change)**; **automatic Morise e-Stamp on a customer-acknowledged delivery (Update 94 — implements the spec-only Update 66: new `POST /api/v1/customer-portal/orders/:id/delivery/acknowledge` [`good` / `damaged` / `incomplete` / `not_received`]; `good` auto-stamps the order's invoice [`invoices.stamped_at` / unique `stamp_number` `MOR-ESTAMP-YYYY-NNNNNN` / `stamp_condition`, migration `20260903180000_invoice_estamp_delivery_ack`] and the invoice PDF draws a navy / amber Morise seal; anything else opens a delivery exception [`deliveries.exception_opened_at`] with no stamp; storefront confirm-receipt panel + `Morise e-Stamp` badge; audited [`invoice.estamp.issued` / `delivery.exception_opened`] + staff notification, no new permission)**; **a clear "Confirm receipt" button / link for the e-Stamp confirmation (Update 95 — the Update 94 acknowledgement is surfaced as a first-class call to action: new read `GET /api/v1/customer-portal/orders/awaiting-confirmation`, `deliveriesToConfirm` / `firstDeliveryToConfirmOrderId` on the dashboard summary, a dedicated `/orders/:id/confirm` storefront page, a `Confirm receipt →` link on each pending order row, and a Home prompt card; no schema or permission change, the acknowledge endpoint + e-Stamp unchanged)**; **confirm-receipt button driven by the order's delivered status (Update 96 — refinement of Updates 94–95: the customer's Confirm-receipt button is driven by the order's status set by admin staff and works for any delivered order, a Morise Logistics delivery record optional; `Order` gains `delivered_at` + `customer_receipt_confirmed_at` / `_condition` / `_note` [migration `20260903200000_order_customer_receipt`]; `POST .../delivery/acknowledge` is order-gated — `good` stamps the order's invoice + mirrors onto a delivery record if present, else a dispute; `awaiting-confirmation` + `deliveriesToConfirm` become order-driven; no new endpoint or permission)**; **social media publishing — composer, publishing engine & status dashboard (Update 97 — a mini Buffer / Hootsuite: new `src/social-publishing` module at `/api/v1/social` with `social_connections` / `social_posts` / `social_post_targets` [migration `20260903220000_social_publishing`], one master post fanned out to many channels in one click with per-platform caption overrides, publish now / schedule / retry / run-scheduled and a status dashboard; three governance permissions `social.post.manage` / `social.post.publish` / `social.post.viewAll` with a drafts-vs-publish split; a four-tab Social Publishing admin screen; the live platform integration [real OAuth, platform APIs, a job queue, media processing] is simulated; new `docx/24`; `openapi.json` → 378 paths / 481 operations)**); **product catalogue for Morise Milling Ltd + branch-tagged product registration (Update 98 — 24 SKUs across 7 categories seeded in `prisma/seed.ts` for Morise Milling Ltd [maize / wheat / millet & sorghum flour, by-products & animal feed, raw grain intake, packaging materials, milling services], `trackBatches` + min/max levels per docx/23 §2.2 and §2.5–2.6; raw-grain/packaging items carry no `unitPrice` so they stay off the storefront while still tracked in Admin Inventory; `Product.branchId` [present since 29 Aug 2026] made settable from the Admin Products screen itself via a new optional `branchId` on `CreateProductDto`/`UpdateProductDto`, validated against the product's own company by a new `assertReferencesBelongTo`; the Admin Products screen gains a subsidiary filter, Company/Branch dropdown pickers, Branch/Stock columns and a first Edit Product modal on `PATCH /products/:id` [live since Sprint 4, no screen before]; no new endpoint, migration or permission; `openapi.json` regenerated unchanged at 378 paths / 481 operations / 188 schemas; group catalogue → 58 products / 25 categories, all six subsidiaries now selling)**); **Morise Research & Consultancy Ltd — seventh subsidiary + RC-MIS specification (Update 99 — a seventh wholly-owned subsidiary [research & consultancy services] seeded in `prisma/seed.ts` from the client attachment `Research_and_Consultancy_Software_Development_Features.docx`: company `a1…008` [Kololo, Kampala], 5 field offices [`b1…0050`–`0054`], 12 departments [`d1…0080`–`0091`] condensing the client's 14-node RC-MIS architecture, 6 starter policies [`p1…0050`–`0055`, ethics/data-protection/fieldwork-safety/data-quality/conflict-of-interest/contract-compliance]; seed-only, no code/schema/permission change; not a storefront seller [0 products — billed on contracts/timesheets]; new `docx/25` restates the client's 14 feature modules, NFRs, architecture, integration points, 14 user roles, tech stack, org-as-built and a 5-phase roadmap; `corporate.js` gains a `SUBSIDIARY_META`/`SUBSIDIARY_BANNER`/News entry; new indigo-accent `subsidiary-research-consultancy.svg` mirrored into both apps; group → 7 subsidiaries / 31 sites-branches)**); **services catalogue for Morise Research & Consultancy Ltd (Update 100 — following the Morise Collateral Management Ltd precedent [contract-billed but carrying a priced catalogue since Update 60], `prisma/seed.ts` gains 23 services [`RC-7001`–`RC-7051`] across 6 categories [Research & Evaluation Studies, Field Data Collection, Data Management & Analysis, M&E Services, Technical & Policy Consultancy, Training & Capacity Building], reusing the existing `ensureService()`/`ensureCategory()` helpers; every line is `service` with a `unitPrice`, so the whole catalogue is storefront-visible; Kampala HQ holds study design/analysis/policy-advisory/day-rate, fieldwork-heavy lines sit at the Gulu/Mbale/Mbarara/Karamoja field offices; seed-only, no schema/code/endpoint/permission change; verified end to end via Postgres + a screenshot-checked Admin Products screen; `openapi.json` regenerated byte-identical; group catalogue → 81 products / 31 categories, all seven subsidiaries now selling)**); **dockerized backend, Admin app and Customer Storefront (Update 101 — `backend/Dockerfile` two-stage build on `node:20-bookworm-slim` for both stages [not Alpine, to avoid a musl/glibc Prisma query-engine mismatch]: `npm ci` → `prisma generate` → `npm run build` [`nest build`, preserving Swagger schema generation]; runtime stage ships only `dist/`, the generated Prisma Client and production `node_modules`; `frontend`/`storefront` Dockerfiles build the Vite bundle on `node:20-alpine` and serve it via `nginx:1.27-alpine`, reverse-proxying `/api/*` to the `backend` container by Compose service name [mirrors each app's dev-server proxy, no CORS change]; `docker-compose.yml` gains all three services, `backend` gated on a new Postgres healthcheck, `frontend`/`storefront` kept on their existing host ports 5173/5174; migrations/seeding stay a host-run step, unchanged; no schema/endpoint/permission change, `openapi.json` unchanged; verified end to end — all three images build clean, all five containers come up healthy, seeded data survives a Postgres container recreate [named volume], a login/page load through each frontend succeeds via the nginx proxy with zero console errors)**); **SSL/TLS certificates via Let's Encrypt (Update 106 — Netlify and Render already auto-provision/auto-renew Let's Encrypt certificates for every site they host, including future custom domains, so nothing was needed there; the self-hosted `docker-compose.yml` stack had no TLS, so a new `mbms/deploy/` directory adds a `nginx-proxy` + `certbot` override [`docker-compose.ssl.yml`], a templated per-app nginx config with HTTP→HTTPS redirect and TLS 1.2/1.3 + HSTS, and an `init-letsencrypt.sh` bootstrap script for the HTTP-01 webroot challenge; infrastructure only, no application/schema/endpoint/permission change)**). Docs 19–22 and 24 carry one-line addenda for the API-docs, public-site, restyle, content-page, social-channels, FAQ/newsletter, orders-localisation, final-edges, collateral-subsidiary, disclaimer-gate, pre-login-localisation, collateral-services, MD-authority, brand-mark, banner-family, banners-on-storefront, delivery, stamped-invoice, persona-assignment, delivery-activation, storefront-wording, payroll-spec, payroll-salary-structures, accounting-finance, cash-bank, budget-mgmt, sales-mgmt, procurement, supplier-mgmt, supplier-mgmt-impl, inventory-mgmt, warehouse-mgmt, asset-mgmt, fleet-mgmt, remaining-catalogue, final-objective, payslip-on-approve, alerts-on-bell, bell-visibility, social-shared-record, social-roster, staff-id-cards, staff-id-card-front-back, staff-id-card-back-resized, milling-subsidiary, estamp-on-delivery, confirm-receipt-cta, order-driven-receipt, social-publishing, milling-products-catalogue, research-consultancy-subsidiary, research-consultancy-catalogue and dockerization passes; **doc 21** (*Multi-Site Collateral Management System — Feature Specification*) carries a full "Services Catalogue" addendum, **doc 22** (*Delivery Management System — Feature Specification*, owning entity Morise Logistics Ltd — its Phase 1 back end is coded in MBMS and **activated** as of 31 Aug 2026, permissions seeded + granted) is new and carries "Customer Acknowledgement & Stamped Invoice", "Assigning a Delivery Persona", "Delivery Module Activated" and "Automatic Morise e-Stamp on a Customer-Acknowledged Delivery — Built" and "A Clear 'Confirm Receipt' Button / Link" and "Confirm-Receipt Button Driven by the Order's Delivered Status" addenda, **doc 23** (*Milling ERP and Integrated Business Management System — Feature Specification*, owning entity Morise Milling Ltd) carries a full "Product Catalogue Seeded for Morise Milling Ltd" addendum recording the 24-SKU catalogue and the categories/items table, and **doc 24** (*Social Media Publishing — Feature Specification*) carries a one-line cross-reference addendum; docs 19–24 each carry both the "Morise Research & Consultancy Ltd — Seventh Subsidiary (Doc 25)" and "Services Catalogue for Morise Research & Consultancy Ltd (Doc 25)" cross-reference addenda, and **doc 25** (*Research & Consultancy Management Information System (RC-MIS) — Feature Specification*, owning entity Morise Research & Consultancy Ltd) carries a full "Services Catalogue Seeded for Morise Research & Consultancy Ltd" addendum recording the 23-service catalogue and the categories/items table; docs 19–25 additionally all carry a "Dockerized Backend, Admin App and Customer Storefront" cross-reference addendum; `Multi_Holdings_Limited_Software_Development_Procedures.docx` gains `Implementation Status Note` 26–83; docs 19–25 additionally all carry an "SSL/TLS Certificates via Let's Encrypt" cross-reference addendum.
