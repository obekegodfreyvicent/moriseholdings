...
| Salary structures | **Planned** | only a flat `Employee.grossSalary`; no structure / grade / component model |
| Basic salary | **Partial** | `grossSalary` is the whole figure; no separate "basic" component |
| Allowances | **Planned** | not modelled |
| Overtime | **Planned** | roster hours exist (Shift Scheduling) but don't feed pay; `computePay` has no overtime input |
| Bonuses | **Planned** | not modelled |
...
- **Backlog:** a salary-structure / component model (basic + allowances +
  overtime + bonuses + commissions); typed deductions + a long-term loan
...
- **Documentation only** — no code, schema, endpoint, permission or
  migration change; the built Payroll module is unchanged. Recorded as
  `Implementation Status Update 70` (docs 01–18) / `Update 45` in
  `morise.docx` with a per-document *Impact* line; **Addendum (1 September
  2026) — Payroll Management Feature Specification** on docs 19–21; and
  `Implementation Status Note 47` in
  `Multi_Holdings_Limited_Software_Development_Procedures.docx`. Pre-edit
  copies in `docx_backup59/`.
...
(DATE)` heading rather than rewriting the body. **Forty-five** 27 August – 1 September
