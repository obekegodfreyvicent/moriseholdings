...
| Supplier suspension | **Partial** — blacklist + the `inactive` status cover it; no dedicated time‑bound suspend‑with‑reason |
| Supplier contacts | **Partial** — one `contact_email` / `contact_phone`; no multi‑contact list |
...
| Supplier evaluation · Supplier performance | **Planned** — no scorecard / rating model |
