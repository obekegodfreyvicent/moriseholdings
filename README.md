...
| `docx/` | The companion documentation set — 24 numbered documents (Project Proposal → UI-UX Design; 19 = Clearing & Forwarding spec, 20 = Warehouse Management spec, 21 = Multi-Site Collateral Management spec, 22 = Delivery Management System spec, 23 = Milling ERP & Integrated Business Management System spec, 24 = Social Media Publishing spec), plus `morise.docx`, `Multi_Holdings_Limited_Software_Development_Procedures.docx`, **`How to Run the System (Local Development) - Morise Holdings Limited.docx`** (practical run steps for this proof-of-concept, restating `mbms/README.md`'s "Running it" section in house documentation format — not part of the dated status-log routine), the raw client attachments (`cf.docx`, `WMS_Features_Uganda_Multi_Branch.docx`, `Multi-Site-Collateral-Management-System-Feature-Specification.docx`, `Delivery-Software-Feature-Specification.docx`, `Milling_Company_Software_Development_Features.docx`), and dated backups in `docx_backup*/`. |
...
The document set is **24 numbered documents** plus `morise.docx` and
...
platform APIs, a job queue, media processing) is **simulated**. The
...
(DATE)` heading rather than rewriting the body. **Seventy-three** 27 August – 8 September
2026 changes are recorded that way across docs 01–18 and `morise.docx`
(docs 19–22 carry one-line addenda, doc 23 carries two full addenda, but
neither carries the full status log):
